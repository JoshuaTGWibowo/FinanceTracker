...
const FINANCE_REALTIME_CHANNEL = "finance-tracker-changes";
...
const createRefreshScheduler = (onRefresh: () => Promise<void> | void) => {
  ...
  return { scheduleRefresh, cleanup };
};

export const subscribeToFinanceChanges = (
  onSnapshot: (payload: FinanceSnapshotPayload) => void,
) => {
  if (!isSupabaseConfigured()) {
    return () => {};
  }
  ...
  const { scheduleRefresh, cleanup } = createRefreshScheduler(async () => {
  ...
  const channel = client
    .channel(FINANCE_REALTIME_CHANNEL)
    .on(
      "postgres_changes",
    ...
    .on(
      "postgres_changes",
      { event: "*", schema: "public", table: "budget_goals" },
      scheduleRefresh,
    )
    .on(
      "postgres_changes",
    ...
    .subscribe((status) => {
      if (status === "CHANNEL_ERROR") {
        console.warn("Supabase realtime channel error for finance sync");
      }
    });
