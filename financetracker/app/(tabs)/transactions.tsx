...
export default function TransactionsScreen() {
  ...
  const profileCurrency = useFinanceStore((state) => state.profile.currency);
  ...
  const getTransactionAmountInDisplayCurrency = useCallback(
    (transaction: Transaction) => {
      ...
      if (Object.keys(exchangeRates).length > 0) {
        // Import convertCurrency dynamically
  ...
  const { sections, summary, filteredRecurring } = useMemo(() => {
    ...
    filtered
      ...
      .forEach((transaction) => {
        ...
        if (transaction.type === "income") {
          existing.dailyIncome += convertedAmount;
          existing.dailyNet += convertedAmount;
        } else if (transaction.type === "expense") {
          existing.dailyExpense += convertedAmount;
          existing.dailyNet -= convertedAmount;
        } else if (transaction.type === "transfer" && selectedAccountId) {
          // Transfers: calculate delta with converted amount for specific account view
    ...
    // Calculate balances using converted amounts
    ...
    transactions,
    visibleAccountIds,
    getTransactionAmountInDisplayCurrency,
  ]);
                  ...
                  >
                    ...
                    <View style={styles.transactionRight}>
                      <Text style={styles.transactionAmount(visual.variant)}>
                        {visual.prefix}
                        {formatCurrency(getTransactionAmountInDisplayCurrency(transaction), currency || "USD")}
                      </Text>
            ...
            {filtersExpanded && (
              <View style={styles.filters}>
                ...
                <View style={styles.filterRow}>
                  <TextInput
                    value={minAmount}
                    onChangeText={(value) => setMinAmount(formatRawAmountInput(value, separators, groupingFormatter))}
                  ...
                  <TextInput
                    value={maxAmount}
                    onChangeText={(value) => setMaxAmount(formatRawAmountInput(value, separators, groupingFormatter))}
                    keyboardType="decimal-pad"
                    placeholder="Max"
                    placeholderTextColor={theme.colors.textMuted}
                    style={styles.filterInput}
